...
static void dfltSigHandler(int signum) { FlightComputer::stopSimulatedCycle(); }
...
int main(int argc, char *argv[]) {
  ...
  if (env_hostname) {
    hostname =
        strdup(env_hostname); // Use strdup to allocate memory for hostname
  }
  ...
  int option_index = 0;
  while ((option = getopt_long(argc, argv, "hd:u:a:p", long_options,
                               &option_index)) != -1) {
    switch (option) {
    case 'h':
      print_usage(argv[0]);
  ...
  if (EXIT_RET != EXIT_CODE_OK || !hostname || uplink_port == 0 ||
      downlink_port == 0) {
    fprintf(
        stderr,
        "Missing required parameters. Please provide all required options.\n");
    print_usage(argv[0]);
    return EXIT_RET;
  }
