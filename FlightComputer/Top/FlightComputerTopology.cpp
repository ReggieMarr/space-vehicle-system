...
// sub-signals: 1Hz, 1/2Hz, and 1/4Hz and zero offset for all the dividers
...
// A number of constants are needed for construction of the topology. These are
// specified here.
...
// Ping entries are autocoded, however; this code is not properly exported.
// Thus, it is copied here.
Svc::Health::PingEntry pingEntries[] = {
    {PingEntries::FlightComputer_blockDrv::WARN,
     PingEntries::FlightComputer_blockDrv::FATAL, "blockDrv"},
    {PingEntries::FlightComputer_chanTlm::WARN,
     PingEntries::FlightComputer_chanTlm::FATAL, "chanTlm"},
    {PingEntries::FlightComputer_cmdDisp::WARN,
     PingEntries::FlightComputer_cmdDisp::FATAL, "cmdDisp"},
    {PingEntries::FlightComputer_cmdSeq::WARN,
     PingEntries::FlightComputer_cmdSeq::FATAL, "cmdSeq"},
    {PingEntries::FlightComputer_eventLogger::WARN,
     PingEntries::FlightComputer_eventLogger::FATAL, "eventLogger"},
    {PingEntries::FlightComputer_fileDownlink::WARN,
     PingEntries::FlightComputer_fileDownlink::FATAL, "fileDownlink"},
    {PingEntries::FlightComputer_fileManager::WARN,
     PingEntries::FlightComputer_fileManager::FATAL, "fileManager"},
    {PingEntries::FlightComputer_fileUplink::WARN,
     PingEntries::FlightComputer_fileUplink::FATAL, "fileUplink"},
    {PingEntries::FlightComputer_pingRcvr::WARN,
     PingEntries::FlightComputer_pingRcvr::FATAL, "pingRcvr"},
    {PingEntries::FlightComputer_prmDb::WARN,
     PingEntries::FlightComputer_prmDb::FATAL, "prmDb"},
    {PingEntries::FlightComputer_rateGroup1Comp::WARN,
     PingEntries::FlightComputer_rateGroup1Comp::FATAL, "rateGroup1Comp"},
    {PingEntries::FlightComputer_rateGroup2Comp::WARN,
     PingEntries::FlightComputer_rateGroup2Comp::FATAL, "rateGroup2Comp"},
    {PingEntries::FlightComputer_rateGroup3Comp::WARN,
     PingEntries::FlightComputer_rateGroup3Comp::FATAL, "rateGroup3Comp"},
};
...
void configureTopology() {
  // Command sequencer needs to allocate memory to hold contents of command
  // sequences
  ...
  rateGroup1Comp.configure(rateGroup1Context,
                           FW_NUM_ARRAY_ELEMENTS(rateGroup1Context));
  rateGroup2Comp.configure(rateGroup2Context,
                           FW_NUM_ARRAY_ELEMENTS(rateGroup2Context));
  rateGroup3Comp.configure(rateGroup3Context,
                           FW_NUM_ARRAY_ELEMENTS(rateGroup3Context));

  // File downlink requires some project-derived properties.
  fileDownlink.configure(FILE_DOWNLINK_TIMEOUT, FILE_DOWNLINK_COOLDOWN,
                         FILE_DOWNLINK_CYCLE_TIME,
                         FILE_DOWNLINK_FILE_QUEUE_DEPTH);

  // Parameter database is configured with a database file name, and that file
  // must be initially read. prmDb.configure("PrmDb.dat");
  // prmDb.readParamFile();
  ...
  // Buffer managers need a configured set of buckets and an allocator used to
  // allocate memory for those buckets.
  ...
  commsBufferManager.setup(COMMS_BUFFER_MANAGER_ID, 0, mallocator,
                           commsBuffMgrBins);
...
void setupTopology(const TopologyState &state) {
  configureTopology();
  setup(state);
  // Initialize socket client communication if and only if there is a valid
  // specification
...
void teardownTopology(const TopologyState &state) {
  // Autocoded (active component) task clean-up. Functions provided by topology
  // autocoder.
  stopTasks(state);
  freeThreads(state);
